...
global $dbname;
$dbname=$db;
$mytree->set_parameters($gui,$mode,$dbname,$table,$show_db_identifier,$debug);
...
global $table;	
if ($branches!="")
    $qs="SELECT * FROM    ".$table."  WHERE     parent=".$branches." or ident=".$branches." ORDER BY ident";
else
    $qs="SELECT * FROM   ".$table."  WHERE     parent='-1' ORDER BY ident";
...
if ($branches!=""){
    if($termmesh!="")
        $qs="SELECT * FROM    ".$table."  WHERE     parent=".$branches." and concat(term,':',detail) like '%".$termmesh."%' ORDER BY ident";
     else
        $qs="SELECT * FROM    ".$table."  WHERE     parent=".$branches." ORDER BY ident";
}else{
    if($termmesh!="")
        $qs="SELECT * FROM   ".$table."  WHERE   lower(concat(term,':',detail)) like '%".strtolower($termmesh)."%'  ORDER BY ident";
    else
        $qs="SELECT * FROM   ".$table."  WHERE     parent='-1' ORDER BY ident";
}
//echo "<br>".$qs."<br>";
...
if ($branches!=""){
    ...
    if($termmesh!="")
        ...
        $qs="SELECT * FROM   ".$table."  ORDER BY ident";
}
//echo "<br>".$qs."<br>";
        $rc=mysql_query($qs);
        $num=mysql_numrows($rc) or die(mysql_error());
...
if ($branches!="")
    ...
    while($ar = mysql_fetch_array($rc)){
       ...
       if($i==0  ) {
		...
		if($chkicon==4)$link="?page=unit_summary.php&unit_competence=".$ar['ident'];
		 $node1   = new HTML_TreeNode(array('text' => $ar['term'].":".$ar['detail'], 'link' => $link, 'icon' => $icon, 'expandedIcon' => $expandedIcon, 'expanded' => $expanded), array('onclick' => "alert('foo'); return false", 'onexpand' => "alert('Expanded')"));
        }else{
             if ( $ar['haschild']=="1" ) {
				...
				if($chkicon==4)$link="?page=unit_summary.php&unit_competence=".$ar['ident'];
				if($chkicon==5)$link="?page=element_group.php&element=".$ar['ident'];
