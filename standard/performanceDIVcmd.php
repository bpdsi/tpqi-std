...
function dataManagePerformance(ID){
	...
	if($("#performance_criteria_input_"+ID).val().length>0){
		...
		$.ajax({
			type: "POST",
			url: "standard/performanceCmd.php",
			data: dataString,
			cache: false,
	...
	echo"<table  border='1' class='noSpacing' style='margin-left:auto;margin-right: auto;margin-top: 5px;width:1000px'>
		...
		while($row=mysql_fetch_array($r)):
			...
			<td class=\"edit_td\">
				...
				<textarea cols='110' rows='2' class=\"editbox\" id=\"performance_criteria_input_".$i."\">".$row["performance_criteria"]."</textarea>
			</td>
			<td width='15%'><input type=\"button\" id=\"CmdBtn_".$i."\" value=\"บันทึก\" onClick=\"dataManagePerformance(".$i.");\" ></td>
		</tr>";
		$i++;
		endwhile;
		echo "<tr id=".$i." class=\"edit_perform\" valign='top'>
